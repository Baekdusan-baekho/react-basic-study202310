...
const emailReducer = (state, action) => {
  ...
  // ab를 입력했을 때
  // state = {value: a, isValid: false}
  // action = {type:USER_INPUT, val: ab}
...
const Login = ({ onLogin }) => {
  ...
  */
  ...
  // 입력란을 모두 체크하여 form의 버튼 dissabled를 해제하는
  // 상태변수 forIsValid의 사이드 이펙트를 처리하는 영역
  useEffect(() => {
    ...
    return () => {
      ...
      // console.log(`useEffect: ${enteredEmail}`);
      clearTimeout(timer);
    }; // 재렌더링이 발생할 때 타이머라는 애를 지워줘

    //  이 배열의 상태변수를 넣어주면 그 상태변수가 바뀔 때마다 useEffect를 재실행함.
  }, [emailState, enteredPassword]);
  // enteredEmail, enteredPassword 둘 중 하나가 변할 때마다 useEffect 동작
